#!/usr/bin/env python3
"""Extract educative.io cookies from Chrome's cookie database (macOS).
...
COOKIE_FILE = Path.home() / ".config/go-educative/cookie.txt"
...
def main():
    ...
    try:
        ...
        cursor = conn.execute(
            ...
            "WHERE host_key LIKE '%educative.io%' ORDER BY name"
        )
    ...
    if not cookies:
        print("No educative.io cookies found!", file=sys.stderr)
        sys.exit(1)

    cookie_str = "; ".join(f"{k}={v}" for k, v in sorted(cookies.items()))

    # Check for critical auth cookies
    required = ["flask-auth", "flask-session", "magicbox-auth"]
    missing = [r for r in required if r not in cookies]
    if missing:
        print(f"WARNING: missing critical cookies: {', '.join(missing)}", file=sys.stderr)
